...
class_name Player
#Constantes de movimento básica
const MIN_SPEED = 4.45
...
const WALK_ACCELERATION = 133.59
const RUN_ACCELERATION = 200.0
const WALK_FRICTION = 182.81
const SKID_FRICTION = 365.62

#velocidade de pulo
const JUMP_SPEED = [-240, -240, -300]
const LONG_JUMP_GRAVITY = [450, 421, 562]
const GRAVITY = [1575, 1350, 2025]

const SPEED_THRESHOLDS = [60, 138]

const STOMP_SPEED = 240
const STOMP_SPEED_CAP = -60
...
@onready var points_animation = preload("res://Cenas/points_animation.tscn")
...
func process_input():
	...
	var was_crounching = is_crouching
	...
	if is_crouching != was_crounching:
		_update_tree()
...
func handle_last_collision():
	...
	#mantem a velocidade y quando colide com uma quina
...
func handle_death():
	#começa o timer de morte, animação toca antes de recarregar nivel
...
func spawn_points_animation(body, points):
	var points_instance = points_animation.instantiate()
	points_instance.position = body.position + Vector2(-20, -20)
	get_tree().root.add_child(points_instance)
